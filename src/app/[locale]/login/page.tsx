    ...
    async function signInWithGoogle() {
        try {
            ...
            if (data?.url) {
                ...
                setError(tLogin('errorGoogle'))
                setLoading(false)
            }
        } catch (err) {
            setError(tLogin('errorGeneral'))
            setLoading(false)
        }
    }
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50 px-4">
            <div className="w-full max-w-md">
                ...
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900">{tLogin('title')}</h1>
                    <p className="text-gray-500 mt-2">{tLogin('subtitle')}</p>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                    ...
                    >
                        ...
                        <span>{loading ? tLogin('loggingIn') : tLogin('loginWithGoogle')}</span>
                    </button>
                ...
                <p className="text-center text-gray-400 text-xs mt-6">
                    {tLogin('termsNotice')}{' '}
                    <Link href="/terms" className="text-blue-500 hover:underline">{t('termsOfService')}</Link>
                    {' '}{t('and')}{' '}
                    <Link href="/privacy" className="text-blue-500 hover:underline">{t('privacyPolicy')}</Link>
                </p>
            </div>
        </div>
    )
