...
export default function ShopsList({ shops }: ShopsListProps) {
    ...
    const handleAction = (shopId: string, shopName: string, action: 'verified' | 'rejected') => {
        const title = action === 'verified' ? 'ยืนยันการอนุมัติ' : 'ยืนยันการปฏิเสธ';
        const message = action === 'verified'
            ? `คุณต้องการอนุมัติร้านค้า "${shopName}" ใช่หรือไม่?`
            : `คุณต้องการปฏิเสธร้านค้า "${shopName}" ใช่หรือไม่?`;
        ...
        showConfirm(
            ...
            async () => {
                try {
                    ...
                    showSuccess(action === 'verified' ? 'อนุมัติร้านค้าเรียบร้อย' : 'ปฏิเสธร้านค้าเรียบร้อย');
                    router.refresh();
                } catch (error) {
                    console.error('[ShopsList] Error updating shop status:', error);
                    showError(error instanceof Error ? error.message : 'เกิดข้อผิดพลาดในการอัปเดตสถานะ');
                }
            },
            title
        );
